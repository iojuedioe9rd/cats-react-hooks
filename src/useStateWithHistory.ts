 ...
 * @returns {T), { capacity \}: { capacity: number; \}) => {\}\}
 */
export function useStateWithHistory<T>(
  ...
  const back = useCallback(() => {
    if (pointerRef.current <= 0) {
      throw new Error("back function called when pointer <= 0");
    }
    pointerRef.current--;
  ...
  const forward = useCallback(() => {
    if (pointerRef.current >= historyRef.current.length - 1) {
      throw new Error(
        `forward function called when pointer >= ${historyRef.current.length}`,
      );
    }
    pointerRef.current++;
    setValue(historyRef.current[pointerRef.current]);
  ...
  const go = useCallback((index: number) => {
    if (index < 0 || index >= historyRef.current.length - 1)
      throw new Error(`Invalid index. The index is ${index.toString()}`);
